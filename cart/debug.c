...
static int byte_instruction(const char *name, chunk *chunk,
                            int offset)
{
    uint8_t slot = chunk->ip.bytes[offset + 1];
    printf("%-16s %4d\n", name, slot);
    return offset + 2;
}
...
{
    uint8_t constant = c->ip.bytes[offset + 1];
...
{
    uint16_t jump = (uint16_t)((chunk->ip.bytes[offset + 1] << 8) |
                               (chunk->ip.bytes[offset + 2]));
...
int disassemble_instruction(chunk *c, int offset)
{
    ...
    printf("%d: %04d ", c->lines.bytes[offset], offset);

    switch (c->ip.bytes[offset])
    {
    case OP_CONSTANT:
    ...
    {
        offset++;
        uint8_t constant = c->ip.bytes[offset++];
        ...
        {
            int isLocal = c->ip.bytes[offset++];
            int index = c->ip.bytes[offset++];
            printf("%04d      |                     %s %d\n",
